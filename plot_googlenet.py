...
def parse_log(log_file):
    ...
    loss_pattern = r"Iteration (\d+).*loss = (?P<loss_val>[+-]?(\d+(\.\d*)?|\.\d+)([eE][+-]?\d+)?)"
    losses = []
    loss_iterations = []
    ...
    accuracy_pattern = r"Iteration (?P<iter_num>\d+), Testing net \(#0\)\n[\s\S]*? accuracy = (?P<accuracy>[+-]?(\d+(\.\d*)?|\.\d+)([eE][+-]?\d+)?)"
    accuracies = []
    accuracy_iterations = []
    accuracies_iteration_checkpoints_ind = []

    for r in re.findall(accuracy_pattern, log, re.MULTILINE | re.DOTALL):
        iteration = int(r[0])
